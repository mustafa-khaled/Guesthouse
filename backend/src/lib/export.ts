...
export interface ExportOptions {
  filename: string;
  fields?: Array<string | { label: string; value: string | ((row: any) => string) }>;
  transforms?: ((item: any) => any)[];
  flatten?: boolean;
  flattenSeparator?: string;
}

export function toCSV<T extends object>(data: T[], options: Partial<ExportOptions> = {}): string {
...
export const bookingExportFields = [
  ...
  {
    label: 'Guest Name',
    value: (row: any) => (row.guestId ? `${row.guestId.firstName} ${row.guestId.lastName}` : ''),
  },
