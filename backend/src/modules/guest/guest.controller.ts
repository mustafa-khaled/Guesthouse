...
export const guestController = {
  create: wrapController({ body: createGuestSchema.shape.body }, async ({ res, data }) => {
    const guest = await guestService.create(data.body);
    return created(res, guest, 'Guest created successfully');
  }),

  getById: wrapController({ params: getGuestSchema.shape.params }, async ({ res, data }) => {
  ...
  delete: wrapController({ params: getGuestSchema.shape.params }, async ({ res, data }) => {
    await guestService.delete(data.params.id);
    return okMessage(res, 'Guest deleted successfully');
  }),

  list: wrapController({ query: listGuestsSchema.shape.query }, async ({ res, data }) => {
    const { page, limit, sortBy, sortOrder, ...filters } = data.query;
    const guests = await guestService.list(filters, page, limit, sortBy, sortOrder);
    return okPaginated(res, guests);
  }),

  getBookings: wrapController({ params: getGuestSchema.shape.params }, async ({ res, data }) => {
  ...
  linkUser: wrapController(
    ...
    async ({ res, data }) => {
      const guest = await guestService.linkToUser(data.params.id, data.body.userId);
