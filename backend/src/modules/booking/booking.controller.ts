...
export const bookingController = {
  ...
  getById: wrapController(
    ...
    async ({ res, data, user }) => {
      const booking = await bookingService.findById(data.params.id, user);
  ...
  cancel: wrapController(
    ...
    async ({ res, data, user }) => {
      const booking = await bookingService.cancel(data.params.id, data.body.reason, user?.id, user);
  ...
  checkIn: wrapController(
    ...
    async ({ res, data, user }) => {
      const booking = await bookingService.checkIn(data.params.id, data.body.roomId, user!.id);
      return ok(res, booking, 'Guest checked in successfully');
    },
  ),

  checkOut: wrapController({ params: checkOutSchema.shape.params }, async ({ res, data, user }) => {
  ...
  assignRoom: wrapController(
    ...
    async ({ res, data }) => {
      const booking = await bookingService.assignRoom(data.params.id, data.body.roomId);
      return ok(res, booking, 'Room assigned successfully');
    },
  ),

  confirm: wrapController({ params: getBookingSchema.shape.params }, async ({ res, data }) => {
    const booking = await bookingService.confirmBooking(data.params.id);
    return ok(res, booking, 'Booking confirmed successfully');
  }),

  list: wrapController({ query: listBookingsSchema.shape.query }, async ({ res, data }) => {
    const { page, limit, sortBy, sortOrder, ...filters } = data.query;
    const bookings = await bookingService.list(filters, page, limit, sortBy, sortOrder);
  ...
  getAddOns: wrapController(
    { params: getBookingSchema.shape.params },
    async ({ res, data, user }) => {
      const addOns = await bookingService.getBookingAddOns(data.params.id, user);
