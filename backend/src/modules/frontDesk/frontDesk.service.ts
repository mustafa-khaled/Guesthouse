...
class FrontDeskService {
  ...
  async getRoomRack(propertyId: string): Promise<{
    ...
    const summary = {
      ...
      available: rooms.filter(
        (r) =>
          !r.isOccupied && (r.status === RoomStatus.CLEAN || r.status === RoomStatus.INSPECTED),
      ).length,
  ...
  async extendStay(bookingId: string, newCheckOut: string, userId: string): Promise<IBooking> {
    const booking = await bookingService.findById(bookingId);
