...
export const propertyController = {
  create: wrapController({ body: createPropertySchema.shape.body }, async ({ res, data, user }) => {
  ...
  getById: wrapController({ params: getPropertySchema.shape.params }, async ({ res, data }) => {
  ...
  delete: wrapController({ params: getPropertySchema.shape.params }, async ({ res, data }) => {
    await propertyService.delete(data.params.id);
    return okMessage(res, 'Property deleted successfully');
  }),

  list: wrapController({ query: listPropertiesSchema.shape.query }, async ({ res, data }) => {
  ...
  getStats: wrapController({ params: getPropertySchema.shape.params }, async ({ res, data }) => {
