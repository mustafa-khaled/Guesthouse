...
export const bookingMutations = {
  ...
  confirm: () =>
    mutationOptions({
      mutationFn: (id: string) => clientFetch(`/v1/bookings/${id}/confirm`, { method: 'POST' }),
    }),

  checkIn: () =>
    mutationOptions({
      mutationFn: (id: string) => clientFetch(`/v1/bookings/${id}/check-in`, { method: 'POST' }),
    }),

  checkOut: () =>
    mutationOptions({
      mutationFn: (id: string) => clientFetch(`/v1/bookings/${id}/check-out`, { method: 'POST' }),
    }),
  ...
  hold: () =>
    mutationOptions({
      mutationFn: (data: Record<string, unknown>) =>
        clientFetch<{ data: { holdId: string; expiresAt: string } }>('/v1/inventory/hold', {
          method: 'POST',
          body: JSON.stringify(data),
        }),
    }),
  ...
  createIntent: () =>
    mutationOptions({
      mutationFn: (bookingId: string) =>
        ...
        }>(`/v1/bookings/${bookingId}/payments/intent`, { method: 'POST' }),
    }),
