...
export default function MyReviewsPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12">
      ...
      {isError && <p className="text-red-600">Failed to load reviews. Please try again.</p>}
      ...
      <div className="space-y-4">
        {reviews?.map((review) => {
          const text =
            (review as { comment?: string; text?: string; review?: string }).comment ||
          ...
          return (
            <Card key={getId(review)}>
              <CardHeader className="flex flex-row items-start justify-between pb-2">
                <div>
                  <CardTitle className="text-base">{getPropertyName(review.propertyId)}</CardTitle>
                  {review.title && (
                ...
                <div className="flex flex-col items-end gap-1">
                  ...
                  {review.status && <Badge variant="default">{review.status}</Badge>}
                </div>
              </CardHeader>
              <CardContent>
